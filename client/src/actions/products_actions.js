...
export function getProductsByArrival() {
  ...
  const request = axios
    .get(`${PRODUCT_SERVER}/articles?createdAt=sold&order=desc&limit=4`)
...
export function addLifestyle(dataToSubmit, existingLifestyles) {
  const request = axios
    .post(`${PRODUCT_SERVER}/lifestyle`, dataToSubmit)
    .then(response => {
      let lifestyles = [...existingLifestyles, response.data.lifestyle];
      return {
        success: response.data.success,
        lifestyles
      };
...
export function getLifestyles() {
  const request = axios
    .get(`${PRODUCT_SERVER}/lifestyles`)
    .then(response => response.data);
